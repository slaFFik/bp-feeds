...
class BPRF_Feed {
	...
	function __construct( $url, $source ) {
		...
		remove_filter( 'wp_feed_cache_transient_lifetime', 'bprf_feed_cache_lifetime', 10 );
	...
	/**
	 * In case we are out of source - display local feed
	 * @return object
	 */
	...
	/**
	 * Use WordPress powered date-time format
	 *
	 * @param string $sep Separator between date and time
	 *
	 * @return string
	 */
	function get_item_date_format( $sep = ' @ ' ) {
		$date = get_option( 'date_format' );
		$time = get_option( 'time_format' );

		return $date . apply_filters( 'bprf_item_datetime_separator', $sep ) . $time;
	}
